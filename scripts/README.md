...
| `genera-screenshot-manifest.mjs` | Genera le due **screenshot del manifest** (M11): il riquadro che Chrome mostra nel foglio di installazione. Le scatta dall'app vera a densità alta — 393×852 @3x per il telefono (`form_factor: narrow`), 1440×900 per lo schermo largo — e stampa le misure reali: se cambiano vanno riallineate in `app/manifest.ts`, perché il browser scarta una screenshot la cui dichiarazione non combacia col PNG. **Rilanciare quando cambia la pelle o la board** (col dev server acceso e `tests/.auth-state.json` presente). | `node scripts/genera-screenshot-manifest.mjs` (base: `E2E_BASE_URL`, default `http://localhost:3000`) |
