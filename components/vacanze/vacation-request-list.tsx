 ...
 * La vista «⛓ A catena» raggruppa per catena: un gruppo per giro, la pillola
...
export function VacationRequestList({ isSecondary, effectiveUserId, loggedInUserId, myPeriodThisYear, year, highlightRequestIds = [] }: Props) {
  ...
  // Le due chip PERSONALI: «Compatibili» (scambio diretto a due) e «⛓ A catena»
  // (giri chiusi da ≥ 3 persone). Un solo filtro attivo per volta; il manager
          ...
          {isManagerView ? (
            ...
            <div className="flex gap-2 overflow-x-auto pb-3 mb-1 no-scrollbar">
              <FilterChip
                ...
                className={!possoFiltrare(propri.length, ipotesi) ? 'opacity-50 cursor-not-allowed' : undefined}
