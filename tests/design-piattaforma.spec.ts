...
/**
 * Lo stesso valore, con i NUMERI normalizzati nel formato.
 *
 * I motori riscrivono i numeri quando li servono da una custom property: `0.03`
 * torna `.03`, `-0.5` torna `-.5`. È una riscrittura legittima — il valore è lo
 * stesso — ma rompe un confronto fra stringhe, e il confronto è tutto quello che
 * questi test hanno. Qui si normalizza il FORMATO, non il merito: le cifre
 * restano quelle che il foglio dichiara, e un `0.03` scritto `0.031` resta un
 * fallimento.
 */
function normalizzaNumeri(valore: string): string {
  return valore.replace(/-?\d*\.?\d+/g, (n) => String(Number(n)))
}

/**
 * Una durata in millisecondi, qualunque unità abbia scelto il motore.
 *
 * Le durate delle molle sono token, quindi passano dal motore: dichiarate `280ms`
 * tornano `.28s`, perché i motori riscrivono i tempi in secondi. Confrontarle
 * come stringhe sarebbe confrontare due modi di scrivere lo stesso tempo; qui si
 * confronta il tempo.
 */
function msDi(valore: string): number {
  const trovato = /^([\d.]+)(ms|s)$/.exec(valore.trim())
  if (!trovato) throw new Error(`durata non riconosciuta: «${valore}»`)
  const n = Number(trovato[1])
  return trovato[2] === 's' ? n * 1000 : n
}
...
test.describe('Moto: le molle sono quelle calcolate, e chi chiede meno movimento le ottiene', () => {
  /**
   * La skin CSS in gioco. Il desktop NON ha una skin: sta sui valori di base,
   * ed è l'unica piattaforma che non deve muoversi — quindi l'unica dove una
   * molla campionata sarebbe un errore invece che una promessa.
   */
  ...
  test('ogni molla del foglio di stile è quella di lib/motion.ts, e la durata è il suo assestamento', async ({
    ...
    if (suDesktop) {
      // Il blocco di base dichiara le tre molle come ALIAS dell'easing di
      // piattaforma: nessun campione, nessun rimbalzo. Si prova leggendo
      // l'easing di base dalla pagina invece di riscriverlo qui — se un giorno
      // qualcuno mettesse una molla campionata nel blocco di base, il desktop
      // inizierebbe a muoversi e questo test lo direbbe.
      const base = normalizzaNumeri(await token(page, '--motion-ease-standard'))
      for (const voce of TOKEN_MOTO) {
        const valore = normalizzaNumeri(await token(page, voce.token))
        expect(valore, `${voce.token} (desktop): nessuna molla campionata`).not.toContain('linear(')
        expect(valore, `${voce.token} (desktop): alias dell'easing di base`).toBe(base)
      }
      return
    }

    for (const voce of TOKEN_MOTO) {
      expect(normalizzaNumeri(await token(page, voce.token)), `${voce.token} (${skin()})`).toBe(
        normalizzaNumeri(linearDaMolla(voce[skin()])),
      )
    }
    ...
    expect(msDi(await token(page, '--motion-duration-enter')), 'il pannello che sale').toBe(
      assestamentoMs(molle.pop[skin()]),
    )
    expect(msDi(await token(page, '--motion-duration-exit')), 'il velo che si accende').toBe(
      assestamentoMs(molle.fade[skin()]),
    )
    expect(msDi(await token(page, '--motion-duration-press')), 'la pressione e il suo ritorno').toBe(
      assestamentoMs(molle.press[skin()]),
    )
  })
  ...
  test('la pressione risponde alla molla su ENTRAMBE le skin', async ({ page }) => {
    ...
    //
    // Si raccolgono le regole che CONSUMANO la molla, non quelle che contengono
    // una parola chiave: il primo tentativo filtrava per `:active`/`::after` e la
    // regola iOS — scritta su `[data-slot='button']` e basta — non veniva mai
    // pescata, quindi il test falliva su una regola che esisteva. Il contratto
    // vero è «chi legge la molla», e quello si cerca.
    const consumatori = await page.evaluate(() => {
      const letture: string[] = []
      const cammina = (lista: CSSRuleList) => {
        for (const regola of Array.from(lista)) {
          const testo = regola.cssText
          if (testo.includes('var(--motion-spring-press)')) letture.push(testo)
          const annidate = (regola as CSSGroupingRule).cssRules
          if (annidate) cammina(annidate)
        }
      }
      for (const foglio of Array.from(document.styleSheets)) {
        try {
          cammina(foglio.cssRules)
        } catch {
          continue
        }
      }
    ...
    expect(consumatori, `la pressione ${skin()} deve leggere la molla`).toContain(
      'var(--motion-spring-press)',
    )
    expect(consumatori, `la molla della pressione su ${skin()} deve governare ${atteso}`).toMatch(
  ...
  test('con «riduci movimento» le molle smettono di rimbalzare', async ({ page }) => {
    ...
    for (const voce of TOKEN_MOTO) {
      ...
      if (!suDesktop)
        expect(normalizzaNumeri(valore), `${voce.token}: collassa su --motion-ease-standard`).toBe(
          normalizzaNumeri(EASING_OSSERVATO[skin()]),
        )
    }
  })
})
