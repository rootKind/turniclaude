...
| overlay e conferme (M3) | `http://localhost:3000/tuoturno`, `/notifiche`, `/vacanze`, `/admin` | `overlay-piattaforma.spec.ts`: un TASK è un **foglio** su iPhone (toccà il fondo, scriminatura, «Chiudi» scritto) e dialog centrato su Pixel/desktop (raggio 28dp, velo 32% senza sfocatura — desktop: valori di base, zero pixel spostati); un ALLARME è centrato su OGNI piattaforma, non ha ×, e «Annulla» non cancella; la card ferie non ha più la conferma in linea e «Elimina» chiede col suo allarme (banco finto via `page.route`, il DELETE è intercettato: nessuna scrittura); le conferme del pannello notifiche non sono più `confirm()` del browser (un listener sui dialog nativi fallisce se ne compare uno) e la domanda sta sopra il pannello già aperto; lo snackbar Android sta in basso e sopra la barra. Gira su tre progetti |
...
| controlli (M4) | `http://localhost:3000/admin`, `/dashboard`, `/tuoturno` | `controlli-piattaforma.spec.ts`: le due skin confrontate sullo STESSO motore con l'override `?platform=` — lo switch (toggle iOS 51×31 verde con pollice fisso / switch M3 52×32 col contorno da 2dp e pollice che cresce / desktop 32×18.4 com'era; solo MISURE, mai premuto: premere scrive nel DB), il campo di testo (inserto senza bordo e angoli continui su iOS, pieno con angoli 4dp/0 e sottolineatura su Android), le chip (32pt/13 su iOS, 32dp/14sp contorno pieno su Android, 28px/12 sul desktop), lo state layer + il ripple che parte dal punto del dito (solo Android; su WebKit touch `:active` sintetico non esiste — si prova sui progetti col mouse), e la rinomina dell'omonimo in un foglio con campo invece di `window.prompt` (banco finto: albero squadre e anagraica intercettati, PUT registrato). Gira su tre progetti |
...
| board e PWA (M5) | `http://localhost:3000/turnisala`, `/installa` | `board-piattaforma.spec.ts`: la card dati della board (`data-slot="desk-card"`) misurata sul motore vero — desktop 8px e nessuna ombra COM'ERA (le suite pixel-sensitive non muovono un pixel), su telefono raggio 12 con ombra tenera (iOS) o elevazione M3 (Android); /installa mostra «Installa ora» SOLO dove il browser espone `beforeinstallprompt` (Android/Chromium) e su WebKit/iPhone il pulsante NON ESISTE (il controllo che becca un fallback finto) con le istruzioni Safari al suo posto. Gira su tre progetti |
